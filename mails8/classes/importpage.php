...
class ImportPage extends RunnerPage
{
	...
	protected function getBasicReportText( $totalRecords, $addedRecords, $updatedRecords, $isNotLogFile = true, $lineBreak = "<br>", $errorMessages = array(), $unprocessedData = array() )
	{
		...
		$reportText .= mysprintf("%s de %s registros procesados satisfactoriamente.", array($boldBegin.$importedReords.$boldEnd, $boldBegin.$totalRecords.$boldEnd)) .$lineBreak.
		...
		if( $notImportedRecords )
			$reportText.= mysprintf("%s registros procesados con error.", array($boldBegin.$notImportedRecords.$boldEnd));

...
{	
	...
	function fillMembers()
	{
		...
		$sql = "select ". $this->connection->addFieldWrappers( "UserName" )
			...
			." from ". $this->connection->addTableWrappers( "oc_ugmembers" ) 
	...
	function fillGroups()
	{
		...
		$sql = "select ". $grConnection->addFieldWrappers( "GroupID" ) .", ". $grConnection->addFieldWrappers( "Label" )
			." from ". $grConnection->addTableWrappers( "oc_uggroups" ) ." order by ". $grConnection->addFieldWrappers( "Label" );
	...
	{
		// It's expected that $this->tName is equal to 'admin_members' so the page's db connection is used #9875		
		$membersWTableName = $this->connection->addTableWrappers( "oc_ugmembers" );
	...
	function fillFields()
	{
		$this->fields[] = array("name" => "DisplayName", "visible" => 1, "caption" => "Nombre Descriptivo");
		$this->fields[] = array("name" => "Email", "visible" => 1, "caption" => "Correo");
		foreach($this->groups as $idx => $g)
		{

...
function checkTableName($shortTName, $type=false)
{
	...
	if ("objetosdecalendario" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("admin_rights" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("oc_users" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("admin_members" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("admin_users" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	if ("confnotifavanzadas" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("auditoria" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("enviomail" == $shortTName && ($type===false || ($type!==false && $type == 1)))
		return true;
	return false;
}
...
{
	$arr = array();
	$strPerm = GetUserPermissions("oc_clndr_objects");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="oc_clndr_objects";
	}
	...
	$strPerm = GetUserPermissions("oc_users");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="oc_users";
	}
	...
	$strPerm = GetUserPermissions("oc_conf_notif_avanz");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="oc_conf_notif_avanz";
	}
	$strPerm = GetUserPermissions("oc_audit");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="oc_audit";
	}
	$strPerm = GetUserPermissions("enviomail");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="enviomail";
	}
...
function ReadUserPermissions($userID = "")
{
	...
	if($userID != "Guest")
	{
		...
		if(!$caseInsensitiveUsername)
		{
			$sql = "select ".$grConnection->addFieldWrappers( "GroupID" )
				...
				." from ". $grConnection->addTableWrappers( "oc_ugmembers" )
	...
	$sql = "select ". $rConnection->addFieldWrappers( "TableName" )
		...
		." from ". $rConnection->addTableWrappers( "oc_ugrights" )
...
{	
	...
	if(!count($_SESSION["UserRights"]["Guest"]))
		return false;
	if(array_key_exists("oc_clndr_objects",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("admin_rights",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("oc_users",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("admin_members",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("admin_users",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("oc_conf_notif_avanz",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("oc_audit",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("enviomail",$_SESSION["UserRights"]["Guest"]))
		return true;
	return false;
}
...
function AfterFBLogIn($pUsername, $pPassword, &$pageObject = null)
{
	...
	$strSQL = "select * from ".$connection->addTableWrappers("oc_users")
...
{
	global $globalEvents;
	$_SESSION["GroupID"] = $data["uid"];


		$_SESSION["OwnerID"] = $data["uid"];
	$_SESSION["_oc_clndr_objects_OwnerID"] = $data["uid"];
		$_SESSION["_oc_conf_notif_avanz_OwnerID"] = $data["uid"];
		$_SESSION["_enviomail_OwnerID"] = $data["uid"];
...
function isPageLayoutMobile( $templateFileName )
{
	...
	return isMobile() && $ovrd != otUseDesktop || $ovrd == otUseMobile;
}

...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="oc_clndr_objects" && $field=="calendarid" && $ptype=="view")
	{
		$value = '<span style="color:' . $data['calendarcolor'] . '">' . $value . '</span>';;
		return $value;
	}	
				if($table=="oc_clndr_objects" && $field=="calendarid" && $ptype=="list")
	{
		$value = '<span style="color:' . $data['calendarcolor'] . '">' . $value . '</span>';;
		return $value;
	}	
				if($table=="oc_clndr_objects" && $field=="calendarid" && $ptype=="export")
	{
		$value = '<span style="color:' . $data['calendarcolor'] . '">' . $value . '</span>';;
		return $value;
	}	
				if($table=="oc_conf_notif_avanz" && $field=="Hora")
	{
...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="oc_clndr_objects" && $field=="startdate") 
	{
		return strftime("%Y-%m-%d");;
	}
				if($table=="oc_clndr_objects" && $field=="enddate") 
	{
		return strftime("%Y-%m-%d");;
	}
				if($table=="oc_conf_notif_avanz" && $field=="Activa") 
	{
		return 1;
	}
				if($table=="enviomail" && $field=="Activa") 
	{
		return 1;
	}
	return "";
